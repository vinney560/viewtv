...
channel_categories = {v["name"]: v.get("group-title", "General") for _, v in channels.items()}
...
class TVAssistant:
    ...
    def create_prompt_template(self):
        """System prompt defining the assistant's identity and capabilities"""
        return (
            "You are ViewTV Assistant, a helpful TV channel specialist. "
            "Your capabilities:\n"
            "1. Check channel status (online/offline)\n"
            "2. Provide channel information (category, description)\n"
            "3. Recommend channels based on categories\n"
            "4. Compare channels\n"
            "5. Explain channel status issues\n\n"
            "Guidelines:\n"
            "- Be concise (1-2 sentences)\n"
            "- Focus on TV channel topics\n"
            "- If unsure, ask clarifying questions\n"
            "- Use provided channel status data\n"
            "- Current time: {current_time}\n"
            "- Available categories: " + ", ".join(set(channel_categories.values())) + "\n\n"
    ...
    def check_channel_status(self, channel_name):
        ...
        for key, data in channels.items():
            if data["name"].lower() == channel_name.lower():
                try:
                    r = requests.get(data["url"], timeout=CHECK_TIMEOUT)
    ...
    def get_channel_info(self, channel_name):
        ...
        for data in channels.values():
            if data["name"].lower() == channel_name.lower():
                return (
                    f"Category: {data.get('group-title', 'General')}, "
    ...
    def find_similar_channels(self, channel_name):
        ...
        category = channel_categories.get(channel_name, "")
        if not category:
            return []
        
        return [
            name for name, cat in channel_categories.items() 
            if cat == category and name != channel_name
        ][:3]
    
    def build_prompt(self, user_query, history):
        """Construct the prompt with context and history"""
        # Format conversation history
        history_text = ""
        for timestamp, user, assistant in history:
            history_text += f"{timestamp} - User: {user}\n"
        ...
        for name in channel_names:
            if name.lower() in user_query.lower():
                mentioned_channels.append(name)
        
        # Add channel status information
        channel_status_info = ""
        for channel in mentioned_channels:
            status = self.check_channel_status(channel)
            channel_status_info += f"- {channel} status: {status}\n"
        
        # Add to prompt if we have channel info
        status_context = ""
        if channel_status_info:
            status_context = "\nCurrent Channel Status:\n" + channel_status_info
        
        return self.prompt_template.format(
            current_time=current_time,
            history=history_text + status_context,
            query=user_query
        )
    ...
    def postprocess_response(self, response):
        ...
        # Remove any user-like prefixes
        for prefix in ["User:", "Human:", "Assistant:"]:
...
def index():
    ...
    if request.method == 'POST':
        ...
        prompt = assistant.build_prompt(user_text, session.get('history', []))
...
if __name__ == '__main__':
    ...
    print("Warming up TV Assistant...")
    test_query = "Is ESPN working right now?"
    test_prompt = assistant.build_prompt(test_query, [])
    print(f"Test response: {assistant.generate_response(test_prompt)}")


    start_scrape_scheduler()  # ✅ Schedule the automatic scraping
    app.run(host='0.0.0.0', port=47947, debug=False, threaded=True)